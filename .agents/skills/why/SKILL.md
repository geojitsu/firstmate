...
This is a worker-directed Firstmate skill. Use it only within the accepted task contract. Firstmate owns dispatch, delivery, no-mistakes, isolated worktrees, decision holds, and captain authority. Any suggested parallel worker or reviewer is advisory until Firstmate routes it through the approved dispatch path; never use vendor-specific hidden delegation, cloud agents, or an unapproved model runner.

Query only evidence tools actually available in this Firstmate environment. Use git and gh-axi for GitHub history, report unavailable categories as gaps, and never imply that an absent MCP or source was searched.
...
Pull PR bodies and discussion via `gh-axi` for any substantive commits:

```bash
gh-axi pr view <number> --json title,body,author,createdAt,mergedAt,labels,closingIssuesReferences,comments,reviews
```
...
Before spawning investigators, list the available MCPs from the Firstmate environment. Use the available-tools map when present. Otherwise inspect the `mcps/` directory Firstmate exposes for enabled MCP servers.
...
Source control is always available through git and gh-axi. For the other six, classify using the MCP name, server instructions, tool names, and resource descriptors. If an MCP could fit more than one category, choose the one matching its primary evidence. Record ambiguous cases in the coverage map.
...
Worker config (each):
Use the approved Firstmate worker route when a live tool is needed. Keep investigators read-only and use only tools available through Firstmate's approved route; they must not write anything.
...
Request one investigator per category through Firstmate's approved dispatch path that has a matching MCP. Each owns exactly one tool or MCP.
...
"It's pure feature code, error tracking won't have anything" is **not** sufficient, and neither is "I doubt long-form docs would have this." Run the search; let the null result speak. The cost of an investigator returning empty is one worker. The cost of missing a design doc that actually exists is a wrong answer.
...
Spawn one synthesizer worker:

The synthesizer may spot-verify citations only through tools available to the approved Firstmate worker route.
...
- `references/synthesizer-prompt.md`. Prompt template for the synthesizer worker, including the output format.
