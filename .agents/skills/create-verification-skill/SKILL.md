...
Use the project's actual browser, CLI, or HTTP conventions. Firstmate uses chrome-devtools-axi for browser work, gh-axi for GitHub, and lavish-axi for structured reports. There is no supported iOS or simulator recipe in this skill; record that bounded limitation instead of inventing tooling.


Every serious project needs a scripted way to drive the real app and prove behavior: launch it, exercise a feature the way a user would, and capture evidence. This skill generates that as a project-local skill (the project's own agent-skill directory, such as `.agents/skills/verify-<app>/`) tailored to the repo. You write the generator's output for the next agent, not for a human: it will be read cold, mid-task, by an agent that has never seen the app.

## 1. Interview the repo, not the user

Answer these from the codebase and record what you cannot observe and route any captain-level decision through Firstmate:
