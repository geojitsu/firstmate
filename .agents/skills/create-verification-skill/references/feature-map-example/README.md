...
- Put `chrome-devtools-axi` and the `notes` CLI on `PATH`.
- Run `chrome-devtools-axi doctor` and require the expected URL, data directory, and build revision.
...
- Run browser actions through `chrome-devtools-axi`.
- Run terminal actions through `the project's CLI or PTY harness with <command>`.
