...
The planner returns typed actions, including `CreateDraft`, `UpdateDraft`, `UpdateIssueFields`, `NoChange`, `CloseMissingCard`, `SkipRecreatingDeletedCard`, `HoldDeletedTask`, and `KeepDeletedTombstone`.
...
It does not yet raise the separate jq intake wake for a captain-created card with no matching task, nor clear a stale dispatch marker when it closes a missing card.
