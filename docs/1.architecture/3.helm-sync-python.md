...
It also raises jq's separate "captain added a card with no backlog task, run intake" wake for a never-before-seen orphan task, and distinguishes a board's first-ever sync (no identity cache yet) from an ongoing one so it closes rather than wakes in that case, matching jq's `tsv_existed` check.
It does not yet clear a stale dispatch marker on a closed card, or distinguish in-flight/blocked hold wording from queued.
