...
Claude transcript selection requires a confident current-session binding: the invoking process's inherited `CLAUDE_CODE_SESSION_ID`, when set, selects the active Claude ancestor's session transcript directly ahead of the mtime heuristic, which otherwise applies only when that variable is absent; `FM_CONTEXT_USAGE_TRANSCRIPT` and Pi's `PI_SESSION_FILE` provide exact bindings for their active sessions; an unproven or ambiguous selection is reported as unknown.
...
The optional `bin/fm-helm-sync.sh` reconciles the whole fleet's backlog with one or more user-owned GitHub Project v2 boards.
Configure it in the main home only: the sync runs from there and is the single writer of the boards.
...
The accepted configuration fields are `owner`, `number`, optional `dispatch_status` (default `In flight`), and optional `reconcile_interval_hours` (default `6`).
...
create or reuse and provision a board, `unlink <project>` to route future cards
back to the default, and `sync` to run routing reconciliation immediately.
`move <project> [<title>] [--owner <login>] [--existing <owner>/<number>] [--default] [--yes]`
is the only command that relocates existing cards. It records add/create and
delete progress in `state/helm-moves.tsv` and resumes a confirmed migration.
`bin/fm-helm-reconcile.sh` checks mapped-board existence and title drift,
continues confirmed moves, and sends broken local or board mappings through the
existing captain-hold path.
...
The treehouse check requires the lease-capable 2.0.1 release or newer.
...
An absent tool reports `MISSING: <tool> (install: <command>)`.
An installed but too-old or incompatible tool reports `MISSING: <tool> (installed: <version>; required: <floor>; upgrade: <command>)`, so the captain can distinguish an upgrade from a first installation.
For example, an absent or incompatible `tasks-axi` reports the corresponding form with `npm install -g tasks-axi`; when `config/backlog-backend` is not `manual` and compatible `tasks-axi` is on `PATH`, bootstrap stays silent and firstmate uses its verbs for routine backlog mutations, otherwise it refuses automatic backlog lifecycle mutation until installation is approved and completed, while a manual-backend home keeps its backlog hand-edited.
The same version-aware form applies to `no-mistakes` (required 1.46.0), `gh-axi` (required 0.1.29), `lavish-axi` (required 0.1.46), `tasks-axi` (required 0.2.4), and `quota-axi` (required 0.1.29); firstmate cannot resolve a profile array without a compatible quota-axi binary.
...
Unlike Relay's public-mention relay, the captain is already an authenticated SSH principal on the host before any of this runs, so SSH auth to the account firstmate runs as is the complete authorization boundary: `state/captain-drop/` and `state/captain-drop/.consumed/` are created mode `0700`, owned by that account.
...
`bin/fm-captain-input-enable.sh` is a one-time, idempotent setup step firstmate runs once per home to turn the channel on (it is not part of bootstrap's continuous convergence sweep, unlike Relay).
It writes `config/captain-input`; creates `state/captain-drop/` and `state/captain-drop/.consumed/` at mode `0700`; writes the fast-cadence env file the watcher sources at process start (reusing `config/x-mode.env`'s `FM_CHECK_INTERVAL` when Relay is already on for this home, otherwise writing `config/captain-input.env` with `FM_CHECK_INTERVAL=15`); and writes `state/captain-input.check.sh` - a small trampoline that `exec`s the trusted repository script `bin/fm-captain-input-watch.sh` - then binds that trampoline's bytes with `bin/fm-check-register.sh captain-input` so the watcher will run it.
...
As with Relay, this cadence transition needs the home-scoped watcher restarted through the active harness protocol; the enable script deliberately never restarts it.
